            ...
            <div class="col-md-9">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form method="GET" action="{{ url('/credito') }}" accept-charset="UTF-8" class="navbar-form navbar-right" role="search">
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless">
                                <thead>
                                    <tr>
                                        <th>#</th><th>Monto</th><th>Interes</th><th>Fecha</th><th>Dias</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($credito as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->monto }}</td><td>{{ $item->interes }}</td><td>{{ $item->fecha }}</td><td>{{ $item->dias }}</td>
                                        <td>
                                            ...
                                            <form method="POST" action="{{ url('/credito' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
